...
import { generateTienPhauDocx } from '../utils/docxGeneratorTienPhau';
...
const STORAGE_KEY = 'tienphau_form_data';
...
const TienPhau = () => {
    ...
    const { isEnabled, isConnected, onlineCount, locks, clientId, remoteData, error: wsError, sendState, lockField, unlockField } = useWebSocket('tienphau');
        ...
        switch (name) {
            ...
            case 'haDuoi':
                if (value && (isNaN(value) || value < 50 || value > 250)) {
                    validation = { valid: false, message: 'Giá trị không hợp lệ (50-250)' };
                } else {
    ...
    const handleExport = useCallback(async () => {
        setIsExporting(true);
        try {
            await generateTienPhauDocx(formData);
        } catch (err) {
    ...
    return (
        <div className="page-container" style={{ position: 'relative' }}>
            <div className="glass" style={{
                ...
                <h1 className="text-xl font-bold" style={{ margin: 0 }}>Tiền Phẫu</h1>
            ...
            <FormSection title="II. Khám Bệnh" className="mt-4">
                ...
                <div className="input-group">
                    ...
                    <div className="glass" style={{ padding: '1rem', marginTop: '0.5rem' }}>
                        <label className="text-sm font-bold">a) Cơ năng</label>
                        <textarea name="benhNgoaiCoNang" value={formData.benhNgoaiCoNang} onChange={handleChange} rows={2} placeholder="Triệu chứng cơ năng..." />
                        <label className="text-sm font-bold mt-2">b) Thực thể</label>
                        <textarea name="benhNgoaiThucThe" value={formData.benhNgoaiThucThe} onChange={handleChange} rows={3} placeholder="Triệu chứng thực thể..." />
                        <label className="text-sm font-bold mt-2">c) Các nghiệm pháp</label>
                        <textarea name="benhNgoaiNghiemPhap" value={formData.benhNgoaiNghiemPhap} onChange={handleChange} rows={2} placeholder="Các nghiệm pháp đặc hiệu..." />
                    </div>
                </div>
