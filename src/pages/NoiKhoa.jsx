...
import { generateNoiKhoaDocx } from '../utils/docxGeneratorNoiKhoa';
...
const STORAGE_KEY = 'noikhoa_form_data';
...
const NoiKhoa = () => {
    ...
    const { isEnabled, isConnected, onlineCount, locks, clientId, remoteData, error: wsError, sendState, lockField, unlockField } = useWebSocket('noikhoa');
    const { bmi, phanLoaiBMI } = useBMICalculation(formData.chieuCao, formData.canNang);

    // Sync BMI calculation
    useEffect(() => {
    ...
    // Auto-generate summary với debounce đơn giản
    useEffect(() => {
    ...
    const handleExport = useCallback(async () => {
        setIsExporting(true);
        try {
            await generateNoiKhoaDocx(formData);
        } catch (err) {
    ...
    return (
        <div className="page-container" style={{ position: 'relative' }}>
            ...
            <div className="glass" style={{
                ...
                <h1 className="text-xl font-bold" style={{ margin: 0 }}>Nội Khoa</h1>
