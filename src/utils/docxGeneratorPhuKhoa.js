...
export const generatePhuKhoaDocx = async (data) => {
    try {
        const dateNow = formatDateTime(data?.ngayLamBenhAn || new Date());

        const paraParts = [
            String(data?.paraDuThang || '').trim(),
            String(data?.paraThieuThang || '').trim(),
            String(data?.paraSayNaoHut || '').trim(),
            String(data?.paraConHienCo || '').trim(),
        ];
        const hasAnyPara = paraParts.some(Boolean);
        const paraText = hasAnyPara ? paraParts.map(p => (p || '0')).join('') : '';

        // Handle Kinh Logic for Export
        const formatKinhDate = (dateVal) => {
            if (!dateVal) return "";
        ...
        const kcLine = data.kinhCuoiMode === 'quen'
            ? "- Kinh cuối: Quên"
            : `- Kinh cuối: ${formatKinhDate(data.kinhCuoiDate)}${data.kinhCuoiTinhChat ? `, tính chất: ${data.kinhCuoiTinhChat}` : ""}`;

        const kacLine = data.kinhApCuoiMode === 'quen'
            ? "- Kinh áp cuối: Quên"
            : `- Kinh áp cuối: ${formatKinhDate(data.kinhApCuoiDate)}`;
        ...
        const doc = new Document({
            ...
            sections: [{
                ...
                children: [
                    ...
                    ...createLabelValueIf("3. PARA: ", paraText),
                    ...
                    createHeading("4. ", "Các cận lâm sàng đã làm:", { spacing: { before: 40, after: 0 } }),
        ...
        saveAs(blob, `${data.hoTen || 'benhan_phukhoa'}.docx`);
    } catch (err) {
        console.error(err);
